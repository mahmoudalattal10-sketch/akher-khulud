...
import {
    Plus, Building, Hotel as HotelIcon, Star, MapPin, Coffee, Utensils, Wifi, Edit2, Trash2, Save, X,
    ...
    UtensilsCrossed, CalendarDays, PlusCircle, CalendarClock, History, CheckCircle2,
...
import api, { HotelsAPI } from '../../../services/api';
import ImageUploader from './ImageUploader';
import { getImageUrl } from '../../../utils/imageHelper';

// Modular Admin Components & Constants
import {
...
interface DestinationsTabProps {
    ...
    onCreate?: (hotel: Partial<AdminHotel>) => Promise<{ success: boolean; data?: AdminHotel; error?: string }>;
...
const DestinationsTab: React.FC<DestinationsTabProps> = ({
    ...
    /* New Filter State - Persisted */
    const [searchQuery, setSearchQuery] = useState(() => sessionStorage.getItem('admin_filter_search') || '');
    const [selectedCountry, setSelectedCountry] = useState(() => sessionStorage.getItem('admin_filter_country') || 'all');
    const [selectedCity, setSelectedCity] = useState(() => sessionStorage.getItem('admin_filter_city') || 'all');
    const [filterStatus, setFilterStatus] = useState(() => sessionStorage.getItem('admin_filter_status') || 'all');
    ...
    useEffect(() => {
        if (viewMode === 'grid' && hotels.length > 0) {
            const savedScroll = sessionStorage.getItem('admin_hotel_scroll');
            if (savedScroll) {
                // Slight delay to ensure DOM is ready
                setTimeout(() => {
                    window.scrollTo({
                        top: parseInt(savedScroll, 10),
                        behavior: 'instant' // Instant jump to avoid disorientation
                    });
                    // Optional: clear it if you only want it once per session, 
                    // but resetting it on every edit is safer to keep it persistent while working.
                }, 50);
            }
        }
    }, [viewMode, hotels.length]); // Re-run when switching back to grid

    const saveScrollPosition = () => {
        sessionStorage.setItem('admin_hotel_scroll', window.scrollY.toString());
    };

    // --- FILTER PERSISTENCE LOGIC ---
    useEffect(() => { sessionStorage.setItem('admin_filter_search', searchQuery); }, [searchQuery]);
    useEffect(() => { sessionStorage.setItem('admin_filter_country', selectedCountry); }, [selectedCountry]);
    useEffect(() => { sessionStorage.setItem('admin_filter_city', selectedCity); }, [selectedCity]);
    useEffect(() => { sessionStorage.setItem('admin_filter_status', filterStatus); }, [filterStatus]);
    ...
    const deleteHotel = async (e: React.MouseEvent, id: string | number) => {
        e.stopPropagation();
        saveScrollPosition(); // Save before action
    ...
    const startEditing = (hotel: AdminHotel) => {
        saveScrollPosition(); // Save before entering edit mode
    ...
    const saveChanges = async (keepOpen: boolean = false) => {
        if (editingHotel) {
            ...
            if (isNew && onCreate) {
                const res = await onCreate(hotelToSave);
    ...
    const updateRoomField = (idx: number, field: keyof Room, value: any) => {
        setEditingHotel(prev => {
            ...
            // [FIX] Prevent double views: If a view is selected, remove any matching "view" tags from amenities
            if (field === 'view' && value) {
                const currentAmenities = room.amenities || [];
                // Remove anything containing "مطلة" or "إطلالة" or "View" from the tags if it matches the selected view
                room.amenities = currentAmenities.filter(a =>
                    !a.includes('مطلة') && !a.includes('اطلالة') && !a.includes('إطلالة') && !a.includes('View')
                );
            }
    ...
    const filteredHotels = hotelsArray.filter(h => {
        // 1. Search Query (Enhanced to check Name, City, Location, and ID)
        if (searchQuery) {
            ...
            const matchesId = String(h.id).toLowerCase().includes(query);

            if (!matchesName && !matchesCity && !matchesLocation && !matchesId) return false;
        }

        // 2. Country Filter
        if (selectedCountry !== 'all' && h.country !== selectedCountry) return false;

        // 3. City Filter (Enhanced to match both ID and Label)
        if (selectedCity !== 'all') {
            // Get the label for the selected city ID from the map
            const cityEntry = (CountryCityMap[selectedCountry] || []).find(c => c.id === selectedCity);
            const cityLabel = cityEntry?.label;

            // Match if hotel city is exactly the ID OR the Label (for manual entries)
            const matchesCityId = h.city === selectedCity;
            const matchesCityLabel = cityLabel && h.city === cityLabel;

            if (!matchesCityId && !matchesCityLabel) return false;
        }
        ...
        return (
            <div className="animate-in slide-in-from-left duration-500">
                ...
                {showGalleryPicker !== null && editingHotel && (
                    <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in duration-200" onClick={() => setShowGalleryPicker(null)}>
                                        ...
                                        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                                            <div className="group/input">
                                                <label className="flex items-center gap-2 text-[11px] font-black text-slate-500 uppercase mb-3 mr-2 transition-colors group-focus-within/input:text-primary">
                                                    <Building size={14} /> اسم الفندق (عربي)
                                                </label>
                                                <input
                                                    type="text"
                                            ...
                                            <div className="group/input">
                                                <label className="flex items-center gap-2 text-[11px] font-black text-slate-500 uppercase mb-3 mr-2 transition-colors group-focus-within/input:text-primary">
                                                    <Building size={14} /> اسم الفندق (إنجليزي)
                                                </label>
                                                <input
                                                    type="text"
                                                    value={editingHotel.nameEn || ''}
                                                    onChange={(e) => setEditingHotel({ ...editingHotel, nameEn: e.target.value })}
                                                    placeholder="Name in English..."
                                                    className="w-full px-7 py-5 bg-slate-50/50 border border-slate-200/60 rounded-[1.5rem] font-bold text-slate-700 outline-none transition-all focus:bg-white focus:border-[#0ca678] focus:ring-4 ring-[#0ca678]/5 placeholder:text-slate-300 text-left"
                                                    dir="ltr"
                                                />
                                            </div>
                                            <div className="group/input">
                                        ...
                                        onSave={async (lat, lng) => {
                                            ...
                                            try {
                                                ...
                                                if (result.success) {
                                                    ...
                                                    alert('تعذر حفظ الموقع. يرجى المحاولة مرة أخرى.');
                                                    return false;
                                                }
                                            } catch (e: any) {
                                    ...
                                    <div className="space-y-10 relative z-10">
                                        ...
                                        <div className="group/amenities">
                                            ...
                                            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                                                {ADMIN_HOTEL_AMENITIES.map((item) => (
                                                    <button
                                                        key={item.id}
                    ...
                    {activeEditTab === 'rooms' && (
                        <div className="space-y-12 animate-in fade-in slide-in-from-bottom-4 pb-12">
                            {(editingHotel.rooms || []).map((room, idx) => (
                                <div key={idx} className="bg-white/90 backdrop-blur-xl p-6 md:p-10 rounded-[3rem] md:rounded-[4rem] border-2 border-slate-100 relative group hover:shadow-2xl hover:border-emerald-200 transition-all duration-700 shadow-xl shadow-slate-200/40 focus-within:z-[500] hover:z-[400]">
                                    ...
                                    <div className="flex flex-col gap-8 md:gap-10">
                                        ...
                                        <div className="grid grid-cols-1 xl:grid-cols-12 gap-10">
                                            ...
                                            <div className="xl:col-span-8 space-y-8">
                                                ...
                                                {/* 3. Room Amenities (Tags) Manager */}
                                                <div className="pt-8 border-t border-slate-50">
                                                    <div className="flex items-center justify-between mb-4">
                                                        <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest flex items-center gap-2">
                                                            <CheckCircle2 size={12} className="text-emerald-500" />
                                                            مميزات ومرافق الغرفة (الوسوم)
                                                        </label>
                                                        <span className="text-[8px] font-bold text-slate-300">يساعد في تحسين نتائج البحث</span>
                                                    </div>
                                                    <div className="flex flex-wrap gap-2">
                                                        {ROOM_AMENITIES.map(amenity => (
                                                            <button
                                                                key={amenity.id}
                                                                onClick={() => toggleRoomAmenity(idx, amenity.label)}
                                                                className={`flex items-center gap-2 px-4 py-2.5 rounded-xl border text-[10px] font-black transition-all group/tag ${(room.amenities || []).includes(amenity.label)
                                                                    ? 'bg-emerald-500 border-emerald-500 text-white shadow-md shadow-emerald-100'
                                                                    : 'bg-white border-slate-100 text-slate-400 hover:border-emerald-200 hover:text-emerald-600'
                                                                    }`}
                                                            >
                                                                <span className={(room.amenities || []).includes(amenity.label) ? 'text-white' : 'text-slate-300 group-hover/tag:text-emerald-500'}>
                                                                    {amenity.icon}
                                                                </span>
                                                                {amenity.label}
                                                            </button>
                                                        ))}
                                                        {/* Optional: Simple Custom Tag Input */}
                                                        <div className="relative group/custom">
                                                            <input
                                                                type="text"
                                                                placeholder="إضافة وسم مخصص..."
                                                                className="px-4 py-2.5 rounded-xl border border-slate-100 text-[10px] font-black outline-none focus:border-emerald-500 w-32 transition-all placeholder:text-slate-300"
                                                                onKeyDown={(e) => {
                                                                    if (e.key === 'Enter') {
                                                                        const val = (e.currentTarget as HTMLInputElement).value.trim();
                                                                        if (val) {
                                                                            toggleRoomAmenity(idx, val);
                                                                            (e.currentTarget as HTMLInputElement).value = '';
                                                                        }
                                                                    }
                                                                }}
                                                            />
                                                            <Plus size={12} className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-300 group-hover/custom:text-emerald-500 transition-colors" />
                                                        </div>
                                                    </div>
                                                    <div className="mt-4 flex flex-wrap gap-1.5">
                                                        {(room.amenities || [])
                                                            .filter(a => !ROOM_AMENITIES.some(ra => ra.label === a))
                                                            .map((tag, tIdx) => (
                                                                <span key={tIdx} className="flex items-center gap-1 px-2 py-1 bg-slate-100 text-slate-600 rounded-lg text-[9px] font-bold">
                                                                    {tag}
                                                                    <button onClick={() => toggleRoomAmenity(idx, tag)} className="hover:text-red-500"><X size={10} /></button>
                                                                </span>
                                                            ))}
                                                    </div>
                                                </div>
                                            ...
                                            <div className="grid grid-cols-1 gap-4">
                                                {room.pricingPeriods && room.pricingPeriods.length > 0 ? (
                                                    room.pricingPeriods.map((period) => (
                                                        <div key={period.id} className="grid grid-cols-1 md:grid-cols-10 gap-6 p-6 bg-slate-50/50 rounded-[2.5rem] border border-slate-100 items-center animate-in zoom-in-95 duration-300 relative group/period focus-within:z-[50]">
                                                            <div className="md:col-span-3">
                                                                ...
                                                                <PremiumDatePicker
                                                                    ...
                                                                    onChange={(date) => updatePricingPeriod(idx, period.id, 'startDate', date)}
                                                                />
                                                            </div>
                                                            <div className="md:col-span-3">
                                                                ...
                                                                <PremiumDatePicker
                                                                    ...
                                                                    onChange={(date) => updatePricingPeriod(idx, period.id, 'endDate', date)}
                                                                />
                                                            </div>
                                                            <div className="md:col-span-3">
