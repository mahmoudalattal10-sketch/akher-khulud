...
const Sidebar: React.FC<SidebarProps> = ({ activeTab, setActiveTab, adminUser, hotels = [], isOpen, onClose, unreadMessagesCount = 0 }) => {
    ...
    const getSmartAlerts = () => {
        ...
        // Show alerts for offers ending in the next year
        const ALERT_THRESHOLD_DAYS = 365;
        const ALERT_THRESHOLD_MS = ALERT_THRESHOLD_DAYS * 24 * 60 * 60 * 1000;

        hotels.forEach(hotel => {
            (hotel.rooms || []).forEach((room: any) => {
                // 1. Stock check - Find distinct lowest stock (including 0)
                // We convert to Number to ensure safety
                const availability = Number(room.available);
                if (!isNaN(availability) && availability < lowestStock) {
                    lowestStock = availability;
                    criticalRoom = room;
                    criticalHotel = hotel;
                }
        ...
        if (endingSoonPeriod) {
            ...
            // Dynamic styling based on urgency (e.g., < 7 days is red, otherwise green/amber)
            const daysLeft = Math.ceil(soonestEnd / (1000 * 60 * 60 * 24));
            ...
            alerts.push({
                ...
                message: `المخزون الحالي: ${endingSoonRoom.available || 0} غرف متوفرة`,
                badge: isUrgent ? 'ينتهي قريباً' : 'تنبيه ذكي',
                color: isUrgent ? 'text-rose-500' : 'text-[#0ca678]',
                icon: <Sparkles size={14} className={`${isUrgent ? 'text-rose-500' : 'text-[#0ca678]'} animate-pulse`} />,
                footer: `ينتهي خلال ${formatCountdown(expiryDate)}`,
                expiryDate
            });
        }

        // Add Logic Stock Alert (Always show the lowest one found)
        if (criticalRoom) {
            const isCritical = lowestStock <= 3; // Red if <= 3, Amber otherwise

            alerts.push({
                id: `stock-${criticalRoom.id}`,
                type: isCritical ? 'stock' : 'info', // Use stock type for red styling
                title: `تحديث مخزون ${criticalHotel.name}`,
                message: lowestStock === 0
                    ? `نفذت الكمية تماماً لغرفة ${criticalRoom.name}`
                    : `أقل مخزون في النظام: ${lowestStock} غرف (${criticalRoom.name})`,
                badge: lowestStock === 0 ? 'نفذت الكمية' : (isCritical ? 'مخزون حرج' : 'حالة المخزون'),
                color: isCritical ? 'text-red-500' : 'text-amber-500',
                icon: <Clock size={14} className={`${isCritical ? 'text-red-500' : 'text-amber-500'} animate-pulse`} />,
                footer: isCritical ? `يرجى مراجعة التوفر فوراً` : `متابعة حالة التوفر`
            });
        }
        ...
        <>
            ...
            `}>
                <div className="p-6 flex items-center justify-between">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <span className="block text-[9px] font-bold text-slate-400 uppercase tracking-widest mt-0.5">لخدمات الضيافة العالمية</span>
                        </div>
                    </div>
                    {/* Mobile Close Button */}
                    ...
                    {menuItems.map((item) => (
                        ...
                        >
                            ...
                            {item.id === 'messages' && unreadMessagesCount > 0 && (
                                <div className="absolute left-4 w-5 h-5 rounded-full bg-rose-500 flex items-center justify-center text-[10px] font-black text-white animate-pulse">
                ...
                <div className="p-4 space-y-3">
                    ...
                    <div className="pt-4 border-t border-slate-50 flex flex-col items-center justify-center text-center gap-1.5 opacity-60 hover:opacity-100 transition-opacity">
                        <span className="text-[10px] text-slate-400 font-black uppercase tracking-[0.15em]">Powering Hospitality by</span>
                        <div className="flex items-center gap-2">
                            <div className="w-1.5 h-1.5 rounded-full bg-emerald-500"></div>
                            <span className="text-sm font-black text-slate-900 tracking-tight">ELATTAL CO.</span>
                        </div>
                    </div>
                </div>
            </div>
