...
const getAmenityData = (id: string) => {
  const mapping: Record<string, { label: string; icon: any }> = {
    ...
    'shuttle': { label: 'نقل للحرم 24/7', icon: <Bus size={14} /> },
      ...
      >
        ...
        <div className="flex flex-col justify-center text-right overflow-hidden flex-1">
          ...
          <div className="flex items-center justify-between mt-1">
            ...
            <div className="flex items-center gap-1">
              <span className="text-slate-400 text-[9px] font-bold">{formatHotelDistance(hotel.distanceFromHaram, hotel.city)}</span>
            </div>
          </div>
        </div>
      </div>
          ...
          <div>
            ...
            <div className="mb-6">
              <div className="flex flex-wrap items-center gap-2 mb-3">
                <h3 className="text-2xl md:text-3xl lg:text-3xl font-black text-text leading-tight group-hover:text-gold transition-colors duration-300 tracking-tight">
                  {hotel.name}
                  {hotel.nameEn && hotel.nameEn !== hotel.name && (
                    <span className="text-lg md:text-xl font-bold text-slate-400 font-sans tracking-wide mx-2">
                      ({hotel.nameEn})
                    </span>
                  )}
                </h3>
            ...
            <div className="flex flex-wrap justify-start gap-2 mb-6">
                      ...
                      <div className="flex items-center gap-4">
                        <div>
                          ...
                          <p className="text-[11px] font-bold text-amber-700/80">
                            من {formatDateArabic(meta.availableFrom)} إلى {formatDateArabic(meta.availableTo)}
                          </p>
                        </div>
                      </div>
              ...
              <div className="flex flex-wrap items-center gap-2">
                ...
                <div className="flex items-center gap-1.5 px-2 py-1.5 bg-slate-50/40 border border-slate-100/50 rounded-xl text-[10px] md:text-[11px] font-black text-slate-500 transition-all duration-300 group-hover:bg-white group-hover:border-gold/20 hover:scale-105">
                  ...
                  {formatHotelDistance(hotel.distanceFromHaram, hotel.city)}
                </div>
