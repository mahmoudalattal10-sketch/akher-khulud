...
interface ApiBooking {
    ...
    guestName?: string;
    nationality?: string;
    userName?: string; // [FIX] Flat field from API
    hotelName?: string; // [FIX] Flat field from API
    roomName?: string; // [FIX] Flat field from API
    user?: {
        id: string;
        name: string;
        email: string;
...
const BookingTable: React.FC = () => {
    ...
    const exportReport = () => {
        ...
        const rows = bookings.map(b => [
            b.id.slice(0, 8),
            b.guestName || b.userName || b.user?.name || 'غير معروف',
            b.hotelName || b.room?.hotel?.name || 'غير محدد',
    ...
    const getStatusDisplay = (status: string) => {
        switch (status) {
            case 'CONFIRMED': return { text: 'حجز مكتمل', class: 'bg-[#e6fcf5] text-[#0ca678] border-[#c3fae8]', dot: 'bg-[#0ca678] shadow-[0_0_8px_rgba(12,166,120,0.3)]' };
            case 'PENDING': return { text: 'بانتظار الدفع', class: 'bg-[#fff9ef] text-[#f59e0b] border-[#ffe3b3]', dot: 'bg-[#f59e0b] shadow-[0_0_8px_rgba(245,158,11,0.3)]' };
            case 'CANCELLED': return { text: 'حجز ملغي', class: 'bg-red-50 text-red-600 border-red-100', dot: 'bg-red-500 shadow-[0_0_8px_rgba(239,68,68,0.3)]' };
            case 'FAILED': return { text: 'فشل الدفع', class: 'bg-rose-50 text-rose-600 border-rose-100', dot: 'bg-rose-500 shadow-[0_0_8px_rgba(225,29,72,0.3)]' };
            case 'COMPLETED': return { text: 'تمت الزيارة', class: 'bg-blue-50 text-blue-600 border-blue-100', dot: 'bg-blue-500 shadow-[0_0_8px_rgba(59,130,246,0.3)]' };
            default: return { text: status, class: 'bg-slate-50 text-slate-500 border-slate-200', dot: 'bg-slate-400' };
        }
    };

    const getPaymentStatusDisplay = (status: string, bookingStatus?: string) => {
        if (bookingStatus === 'FAILED') {
            return { text: 'خطأ في الدفع', class: 'bg-rose-600 text-white shadow-rose-100 border-rose-600', icon: <AlertCircle size={12} className="text-white" /> };
        }
        switch (status) {
            ...
            default: return { text: 'بانتظار اتمام الدفع', class: 'bg-[#fff9ef] text-[#f59e0b] border-[#ffe3b3] shadow-sm', icon: <Clock size={12} /> };
        }
    };
                                    ...
                                    <tr key={booking.id} className="bg-white rounded-[2.5rem] hover:shadow-xl hover:shadow-slate-200/50 transition-all duration-500 group relative border-y border-transparent hover:border-emerald-100">
                                        <td className="px-8 py-8 rounded-r-[2.5rem]">
                                            <div className="flex items-center gap-5">
                                                ...
                                                <div>
                                                    <div className="font-black text-slate-800 text-lg leading-tight">{booking.guestName || booking.userName || booking.user?.name || 'غير معروف'}</div>
                                                    ...
                                                    {booking.nationality && (
                                                        <div className="text-[9px] text-gold font-black mt-1 tracking-wider bg-gold/5 px-2 py-0.5 rounded-lg border border-gold/10 ml-2">
                                                            {booking.nationality}
                                                        </div>
                                                    )}
                                                </div>
                                        ...
                                        <td className="px-8 py-8 text-right">
                                            <div className="flex items-center gap-4">
                                                ...
                                                <div>
                                                    <div className="text-sm font-black text-slate-700 leading-tight">{booking.hotelName || booking.room?.hotel?.name || 'فندق'}</div>
                                                    <div className="text-[11px] text-slate-400 font-bold mt-1">{booking.roomName || booking.room?.name || 'غرفة مميزة'}</div>
                                                </div>
                                            </div>
                                        </td>
                                        ...
                                        <td className="px-8 py-8 text-right">
                                            {(() => {
                                                const pStatus = getPaymentStatusDisplay(booking.paymentStatus, booking.status);
                                                return (
                                            ...
                                            <div className="relative inline-block">
                                                ...
                                                {activeMenu === booking.id && (
                                                    <div className="absolute left-0 top-full mt-3 w-64 bg-white rounded-[2rem] border border-slate-100 shadow-2xl z-50 p-3 animate-in fade-in zoom-in-95 origin-top-left">
                                                        ...
                                                        <button
                                                            onClick={() => window.open(`/booking/${booking.id}/voucher`, '_blank')}
                                                            className="w-full flex items-center gap-4 px-5 py-4 text-right text-sm font-black text-slate-600 hover:bg-slate-50 rounded-2xl transition-all"
                                                        >
                                                            <Printer size={20} />
